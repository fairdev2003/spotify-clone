...
const Card = (props: {img: string, playlist_name: string , desc?: string, card_type?: "album" | "playlist" | "popular" }) => {


  const CardTypeChanger = ( type: string | undefined ) => {
    if (type === "album") {return (
      <>
        <h1 className='text-[white] mt-3 text-md font-[600]'>{props.playlist_name}</h1>
        <p className='text-[#525050] text-sm font-[500]'>{props.desc}</p>
      </>
  ...
  return (
    <div className='h-[auto] w-[200px] bg-[#3a3939] p-5 pb-8 rounded-lg relative cursor-pointer hover:bg-[#383737]' id='card-container'>
        ...
        <div>
            {CardTypeChanger(props.card_type)}
        </div>
    </div>
  )
}
